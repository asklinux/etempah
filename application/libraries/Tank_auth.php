...
class Tank_auth {
        ...
        if ((strlen($login) > 0) AND (strlen($password) > 0)) {
            ...
            if (!is_null($user = $this->ci->users->$get_user_func($login))) { // login ok
                ...
                if ($hasher->CheckPassword($password, $user->password)) {  // password ok
                    ...
                    $this->error = array('login' => 'auth_incorrect_password');
                }
    ...
    function logout() {
        ...
        $this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'user_level' => '', 'user_fullname' => '', 'status' => ''));
    ...
    /**
     * Check the user logged in as admin. Also test if user is activated or not.
     *
     * @param	bool
     * @return	bool
     */
    function is_admin($activated = TRUE) {
        $user_level_id = $this->ci->users->get_user_level_by_name("admin");

        return (
                ($this->ci->session->userdata('user_level') == $user_level_id) &&
                ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED)
                );
    }

    /**
     * Check the user logged in as subadmin. Also test if user is activated or not.
     *
     * @param	bool
     * @return	bool
     */
    function is_subadmin($activated = TRUE) {
        $user_level_id = $this->ci->users->get_user_level_by_name("subadmin");
    ...
    /**
     * Check the user logged in as headdriver. Also test if user is activated or not.
     *
     * @param	bool
     * @return	bool
     */
    function is_headdriver($activated = TRUE) {
        $user_level_id = $this->ci->users->get_user_level_by_name("headdriver");
    ...
    /**
     * Check the user logged in as normal user. Also test if user is activated or not.
     *
     * @param	bool
     * @return	bool
     */
    function is_normal_user($activated = TRUE) {
        $user_level_id = $this->ci->users->get_user_level_by_name("user");
     ...
     * @return	array
     */
    function create_user($username, $email, $password, $email_activation, $user_level) {
        if (!$this->ci->users->is_email_available($email)) {
            $this->error = array('email' => 'auth_email_in_use');
        } elseif ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
    ...
    function get_error_message($key = '') {
        if ($key) {
            return isset($this->error[$key]) ? $this->ci->lang->line($this->error[$key]) : false;
        }
        return $this->error;
    }

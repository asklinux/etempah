...
class CI_Pagination {
    ...
    var $first_link = '&lsaquo;&lsaquo; Awal';
    var $next_link = 'Berikut &rsaquo;';
    var $prev_link = '&lsaquo; Sebelum';
    var $last_link = 'Akhir &rsaquo;&rsaquo;';
    var $uri_segment = 3;
    var $full_tag_open = '';
    var $full_tag_close = '';
    var $first_tag_open = '';
    ...
    function create_links() {
        ...
        // And here we go...
        $output = '';
        $output .= '<div class="pagination ' . $this->pagination_align . '"><ul>';
            ...
            for ($loop = $start - 1; $loop <= $end; $loop++) {
                ...
                if ($i >= $base_page) {
                    if ($this->cur_page == $loop) {
                        $output .= '<li class="disabled"><a href="#">' . $this->cur_tag_open . $loop . $this->cur_tag_close . '</a></li>'; // Current page
                    } else {
                        $n = ($i == $base_page) ? '' : $i;
    ...
    function create_pagination_text() {
        if (!empty($this->cur_page)):
            if ($this->cur_page == 1):
                $pagination_text = "Paparan 1 - " . $this->cur_page * $this->per_page . " daripada {$this->total_rows} rekod";
            else:
                $record_range = "";
                if (($this->cur_page * $this->per_page) > $this->total_rows):
                    if (( ($this->cur_page - 1) * ($this->per_page ) + 1 ) == $this->total_rows):
                        $record_range = ( ($this->cur_page - 1) * ($this->per_page ) + 1 );
                    else:
                        $record_range = ( ($this->cur_page - 1) * ($this->per_page ) + 1 ) . " - " . $this->total_rows;
                    endif;
                else:
                    $record_range = ( ($this->cur_page - 1) * ($this->per_page ) + 1 ) . " - " . $this->cur_page * $this->per_page;
                endif;
                $pagination_text = "Paparan {$record_range} daripada {$this->total_rows} rekod";
            endif;
        else:
            $pagination_text = "Paparan 1 - {$this->total_rows} daripada {$this->total_rows} rekod";
        endif;
